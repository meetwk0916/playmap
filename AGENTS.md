...
- `maintainer.html`：仅本地的评分问卷和草稿批量导出；维护机制见 `docs/public-places.md`。
...
- 公开地点、三维评分、独立推荐及本地维护以 `docs/public-places.md` 为现役合同；[早期实施规格](https://github.com/meetwk0916/playmap/issues/31) 保留决策背景。真实名单与头像必须由维护者提供，不能把测试内容当作正式经历发布。
- 维护问卷仅限本地，草稿与个人地图分开存储；保存和导出不等于发布，推荐不能由星级推断。
